	...
	<Row class="margin-top-10">
		...
		title="添加项目">
			<Form ref="formCustom" :model="editProjectData" :rules="ruleCustom" :label-width="80">
				...
				<FormItem label="开源地址" prop="open_url">
					<Input type="text" v-model="editProjectData.open_url"></Input>
				</FormItem>
				<FormItem label="官方网站" prop="site">
					<Input type="text" v-model="editProjectData.site"></Input>
				</FormItem>
				<FormItem label="README地址" prop="readme"><slot name="label">可以是地址也可以是内容</slot>
					<Input type="textarea" v-model="editProjectData.readme" ></Input>
				</FormItem>
				<FormItem label="项目描述" prop="name">
  ...
  data () {
    ...
    onModelOk(formName){
      this.$refs[formName].validate((valid) => {
        if (valid) {
          // 服务器交互
	        this.$axios.post('/api/project', this.editProjectData).then(res => {
	          if(res.success) {
